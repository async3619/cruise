...
import { Rail, Root, Thumb, Track } from "./Slider.styles";
...
export class Slider extends React.PureComponent<SliderProps, SliderStates> {
    ...
    public render() {
        ...
        return (
            <Root onMouseDown={this.handleMouseDown} onMouseUp={this.handleMouseUp} data-testid="Slider">
                <Track ref={this.trackRef}>
                    {!disabled && (
                        <Thumb className="thumb" ref={this.thumbRef} style={{ opacity: dragging ? 1 : undefined }} />
                    )}
                    {!disabled && <Rail className="rail" active={dragging} ref={this.railRef} />}
                </Track>
            </Root>
        );
    }
